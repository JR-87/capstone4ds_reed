...
introduced by D. R. Cox in 1972 [@nahhas2025]. Survival data is data
with a value for time and an event [@Abeysekera2009]. For example, in a
study investigating the survival likelihood of mechanical equipment, the
time value would begin when the equipment begins functioning and would
conclude in the event the equipment stops functioning [@Smith2003]. CPH
models can use single or multiple covariates as a means of creating
predictions until failure and are capable of dealing with survival data
that contains censored data as well [@Seung2023]. Censored data is when
the information about an individual in a study is only known for a
certain period of time, such as the length of the study [@klein2005].
The models also have the advantage of being semi-parametric which allows
more flexibility as compared to other models [@Ming-Chiang2014].
...
CPH requires data to contain a time to event value and have a binary
indicator variable for the event occurrence. The data must not contain
any missing values, but can be data that is censored which CPH is
designed to handle [@nahhas2025]. In addition, the data must meet
several assumptions for CPH to be a valid model to use on the data which
are discussed below.
...
This equation can then be adapted for the specific data being studied.
...
The Kaplan-Meier estimator and curve are frequently used in survival
analysis, especially when dealing with censored data or data where the
time-to-event value fluctuates per subject [@klein2005]. The
Kaplan-Meier estimate was proposed by Edward L. Kaplan and Paul Meier in
1958 to deal with incomplete observations [@rich2010]. It is the
standard estimator of the survival function and is defined by the
following equation:
...
-   $Y_i$ is the number of individuals who are at risk at time $t_i$

for all values of $t$ where there is data. The estimator is not
well-defined for $t$ beyond the largest data value for time
[@klein2005].

The Kaplan-Meier curve on the X-axis represents the length of survival
time on the interval from 0 to the largest observation time while the
Y-axis represents the cumulative probability of surviving at a given
time. The curve is based on step-wise estimates thus making the curve
non-continuous [@rich2010]. Generating a Kaplan-Meier curve requires the
R packages `survival` and `survminer` and uses the survival object and
survival formula for the data. The below curve was generated using the
`survival` package and dataset "lung" and demonstrates the general
survival probability using the `ggsurvplot` function [@R-base].
...
Kaplan-Meier can be used to compare the survival probability of
different groups, such as the sex of an individual. The log rank test
can be used to determine the significance of each group. The test
calculates the $\chi^2$ value for each event time for each group and
sums the results which are then added together to generate the $\chi^2$
value for the full curve for each group [@rich2010]. The hypothesis test
for the log rank test is:

$$
\begin{align*}
H_0 &: \text{in terms of survivability, there is no difference between the two groups} \\
H_1 &: \text{there is a survival difference between the two groups} 
\end{align*}
$$
...
Using an $\alpha = 0.05$ and the same data as the above Kaplan-Meier
curve comparing sex, the $H_0$ hypothesis is rejected as there is enough
evidence to suggest there is a survival difference between males and
females.

While the Kaplan-Meier curve and estimator are useful, there is an
assumption of non-informative censoring when using them. This assumption
means that there is no additional information on the survivial
likelihood for a subject after they have been censored. If that
assumption is violated, then Kaplan-Meier curves will not be estimating
the correct equation [@klein2005].

**CPH Model**

To generate the CPH model, a survival object must first be created.
Using the R package `survival`, the survival formula can be generated by
running `Surv(time, event) ~ x`. That formula is then applied to the R
function `coxph(formula, data=)` which will generate the CPH model
[@R-base]. The output of the CPH model includes the coefficients
(\$\\beta\$ and the confidence intervals for each covariate, and the
concordance value, likelihood ratio test, Wald test, and the score
(logrank) test of the overall model [@nahhas2025].

### Assumptions

For a CPH model to be valid, the following assumptions must be met
either from the raw data or through transformation.
...
individuals involved in studies may have decades before an event is
...
this individual when modeling survival.

**Independence Assumption**

Survival times of observed individuals should be independent of each
other. In an example where survival of plane engines were being
evaluated, if each plane has two engines and the failure of one engine
caused the accelerated failure of another due to increased workload, it
would produce biased conclusions and violate the independence
assumption.

### Model Evaluation

Evaluating a CPH model requires examining the predictors to determine
which ones are significant and generating the concordance index which
will determine how accurate the model is at predicting survival.
