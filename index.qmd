...
reject or fail to reject $H_0$ can be made [@nahhas2025]. 
...
As defined by the formula, cumulative hazard is a summation of the hazard rates for an individual from time 0 to time $t$. A higher cumulative hazard indicates a higher risk of event. Cumulative hazard is used as an additional measure for risk, but it is mainly used to calculate survival probability to plot against time on survival curves. 

The cumulative hazard will be calculated in R by grouping the data by the id field and finding the summation of the hazard rates for the individual through the duration of the observation period. A column will be added showing the cumulative hazard for each individual at ach point in time.
...
The resulting survival probability will be between 0 and 1 with a value close to one indicating a high probability of the individual not experiencing the event. Conversely, a value close to 0 indicates a higher probability of the individual experiencing the event. A survival probability of .05 indicates that only 5% of individuals under the same conditions would survive. Survival probability is an important metric when modeling, visualizing, and making predictions with survival data.
...
A forest plot will be created using the `plot_model()` function from the sjPlot package. The CPH model can be passed as the sole argument to visualize the hazard ratios.
...
```{r}
survfit <- survfit(cox, newdata = test)
plot(survfit, 
     col = "blue", 
     conf.int = TRUE, 
     mark.time = FALSE,
     ...
     main = "Survival Probability Over Time")
```
