...
probablity of machines, machinery maintenance timelines, and insurance
...
using a step-wise regression and comparing the results based on the
...
positive correlation with the hazard ratio and the higher the number the
more severe the effect is on the hazard ratio.
...
normally distributed and the box plot shows there six outliers in the
dataset. The engines have a median survival time of 199 cycles with a
standard deviation of 46.78. The maximum number of cycles an engine
...
best model to use for this analysis, a forward and backward step-wise
regression will be done to generate the models with the best Akaike
Information Criterion (AIC) and Bayesian Information Criterion (BIC)
values and compare the concordance values of those models with models
that have covariates that were statistically significant when used in a
CPH model with the training data. The model with the highest concordance
value, and therefore, the most accurate will be used for the analysis.

The table below provides the model number, the covariates used, the AIC
and BIC from the step-wise regression if applicable, and the concordance
index. The concordance index generates a value to determine which model
is the most accurate with the provided covariates. After determining the
most accurate model, the CPH assumptions will be tested.
...
train_sig <- lapply( univ_formulas, function(x){coxph(x, data = train_grouped)})
...
Based on the above model results, the model that has the highest
concordance value is model 1 which has all of the covariates. However,
before proceeding with that model, a test for linear dependency will be
run to determine all covariates are linearly independent.

```{r}
kable(alias(cox1), format = "markdown")
```

The above result indicates that sm19 needs to be removed from the model
as it linearly dependent on another value. In addition, sm16 resulted in
a possible infinite coefficient so that covariate has also been removed.
The proceeding analysis will be done using model 1 without sm16 and
sm19. The below tables provide the summary values from the model.
...
covariate is shown below. For each covariate, there is no clear slope
...
PH assumption. To ensure the model is as accurate as possible, the model
will addapted without those covariates and the global p-values will be
compared.
...
covariates violate the PH assumption on a local level. As such, the
model will be recreated by removing the covariates that violate the PH
assumption. After recreating the model, the PH assumption and the
linearity assumption will be tested again with the new PH global p-value
being compared to the original model's global p-value.

The table below demonstrates the new model information which has
eighteen covariates: os2, sm1 - sm12, sm15, sm17, sm18, sm20, sm21.

```{r}
cox1.2 <- coxph(data = data_grouped, surv_object ~ os2 + sm1 + sm2 + sm3 + sm4 + sm5 + sm6 + sm7 + sm8 + sm9 + sm10 + sm11 + sm12 + sm15 + sm17 + sm18 + sm20 + sm21)

kable(tidy(cox1.2), format = "markdown")
g <- glance(cox1.2)
g <- g %>% select(n, nevent, concordance, std.error.concordance, logLik, statistic.log, p.value.log, statistic.wald, p.value.wald, statistic.sc, p.value.sc)
kable(g, format = "markdown")
...
hypothesis test and Schoenfeld residuals. The global p-value is now
$< \alpha$ so the PH assumption has not been met for the new model. As
such,, the remaining analysis will be done using the first model that
had a global p-value $>\alpha$ which has all but two covariates.
...
create_ph_plots(cox1.2, 3, 5)

```

If the new model had passed the PH assumption, the linearity assumption
would have been checked again.

### Model Results

**Evaluation**

The CPH model that was selected has a concordance value of `{r} ci`
which indicates that it is more than 50% accurate in predicting the
survival of the machines. However, this model is not completely accurate
so there is an expectation of some error in the model survival
probabilities which will lead to caution in providing results in
...
[@eq-hazardrate], the cumulative hazard value[@eq-cumulativehr], and the
survival probability [@eq-survprob]. The hazard rate at time $t$ will be
calculated for each engine at the engines max time value. The cumulative
hazard value is the total hazard accumulated for the subject over time
and is used to estimate the risk for the subject at a point in time. The
engines that have higher hazard rates or higher cumulative hazard values
are at a greater risk of experiencing failure. The below table provides
the hazard rate and suvivial probability for the first 10 engines in the
dataset.
...
prediction of specific failure times, it does allow for the creation
...
NASA not to use engines after 128 iterations have passed. The engines
