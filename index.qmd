...
plot which shows a low hazard until `t = 128` with a gradual increase. The model's final concordance index of .696 indicates that the calculated confidence interval is fairly accurate, but it is still subject to a substantial margin for error.
...
While the concordance index of .696 achieved in this analysis demonstrates the model is fairly accurate, there is still room for improvement. Survival models resulting in a concordance index of .9 and higher are highly accurate and would provide a tighter, more accurate confidence interval for the longevity of engines. After attempting transformations of the data and creating models many different combinations of covariates, the final c-index that was calculated is the highest value achieved where the covariates were able to meet the proportional hazards model.

As discovered during the literature review, a weakness of the cox proportional hazards model is the necessity to meet the proportional hazards assumption. Due to the difficulty meeting the assumption during this analysis, to achieve a more accurate model it would be beneficial to explore other survival models for this dataset. 
