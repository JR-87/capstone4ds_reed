...
in other fields. This paper will study the remaining useful life (RUL)
of a NASA aircraft engine using data provided by NASA for the study on
propagation modeling [@saxena2008]. In particular, this paper will be
examining the data from the engine make two which has a training dataset
...
kable(head(test, n = 5), format = "markdown", caption = "Testing Data")
...
kable(head(train, n = 5), format = "markdown", caption = 'Training Data')
```
...
number of cycles an engined lasted was 128.
...
kable(summarymetrics, format = "markdown", padding = 0)
...
To determine the intitial survival function, the Kaplan-Meier survival
curve was generated and is shown below. The graph verifies that the
median survival time is roughly 200 cycles and shows the maximum cycle.
After that cycle has been reached, the survival probability plummets to
0%.
...
CPH models are os1, os3, sm1, sm2, sm5, sm6, sm7, sm11, sm12, sm13,
sm14, sm19, sm20, and sm21 which is fourteen out of twenty-four possible
covariates.
...
The model is recreated below with another proportional hazards test. To
improve the model and ensure the proportional hazards assumption is met,
several covariates that do not meet the assumption are removed.
...
The new model has seven covariates with only one not meeting the
proportional hazards assumption. Although sm5 does not have a p value \>
.05, the assumption overall holds for the new model and sm5 was not
removed from the model because of its large impact on the hazard ratio
shown in the forest plot. The new model's full summary is below.
...
covariate is shown above. For each covariate, there is no clear slope
