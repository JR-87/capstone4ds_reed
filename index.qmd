...
COVID-19 study using data from a Pakistani hospital noted that the model
was limited due to the recovery possibility of COVID-19 patients. CPH
assumes as time goes on, the survival probability will approach zero
with no survivors, which was not the case with COVID-19. Though CPH was
not the best model for this study, the results of the model were that
individuals who were asymptomatic and young had a higher chance of
surviving the virus [@asghar2024]. In 2024, a study was done in the US
on the relationship between chest pain occurring and mortality of the
person experiencing chest pain using CPH. A response to that study
demonstrated the various limitations of CPH, with the primary concern
being the constant hazards ratio assumption that CPH relies on. In the
heart study, the effect of triglyceride-glucose was assumed to be a
constant, but it is actually a value that fluctuates over time along
with an individual's body weight and blood glucose levels. As such, the
model would struggle to predict correctly the effect of
triglyceride-glucose given the violation of the constant hazards ratio
[@jiang2024].

An additional limitation of CPH comes from the data as the selection of
covariates and the generation of the data can be biased. Two studies
done using UK health data demonstrated how bias can impact the values
for the covariates when relying on patient-submitted information. The
first was a study on the connection between salt intake and the
development of anxiety or depression after 14.5 years [@wang2025], and
the second was a study on the association between postpartum depression
and the development of two or more chronic diseases [@zhang2025]. Both
of those studies relied on patient-submitted health data and survey
responses supplemented with death records and other accessible health
records. The reliance This created a possible bias in their data that
may have negatively influenced their model as their collected data may
not accurately reflect the true data. These articles provide examples of
...
studies provide examples of CPH models outside the health industry.
...
plot_model(forest,
  ...
  ylab(label = "Estimated Effect on Hazard Ratio")
...
autoplot(km_fit) + theme_bw() + labs(y = "Survival Probability (%)", x = 'Time (Iterations)', title = "Kaplan-Meier Survival Curve")
```
...
$< 0.05$ will be considered statistically significant. The table
displays the covariate, the $\beta$ value, the hazard ratio (HR), the
95% confidence interval for the hazard ratio, and the p-value generated
from the Wald test. As can be seen, none of the covariates are
statistically signification on their own.
...
forms <- sapply(covariates,
                        function(x) as.formula(paste('Surv(time, status)~', x)))

univ_models <- lapply(forms, function(x){coxph(x, data = data_grouped)})
...
To determine the best model to use for this analysis, a forward and
backward stepwise regression was done to generate the models with the
best Akaike Information Criterion (AIC) and Bayesian Information
Criterion (BIC) values. The second and third models were generated using
this regression method. The fourth model was generated using the Lasso
penalized Cox regression method.
...
#input/out for glmnet
all_mod <- model.matrix(Surv(time, status) ~  + os2 + os3 + sm1 + sm2 + sm3 + sm4 + sm5 + sm6 + sm7 + sm8 + sm9 + sm10 + sm11 + sm12 + sm13 + sm14 + sm15 + sm16 + sm17 + sm18 + sm19 + sm20 + sm21, data = data_grouped)[, -1]  

# lasso penalized cox
cvfit <- cv.glmnet(all_mod, surv_object, family = "cox")
coefs <- coef(cvfit, s = "lambda.min")
new_cov <- rownames(coefs)[which(coefs != 0)]
...
#lasso
cox4 <- coxph(data = data_grouped, surv_object ~ os2 + sm1 + sm3 + sm4 + sm9 + sm10 + sm11 + sm14 + sm15 + sm16 + sm17 + sm18)
...
stepwise_results <- data.frame(c("model 1", "model 2", "model 3", "model 4"), 
                               c("All covariates", "os3, sm3, sm4, sm5, sm8, sm9, sm15, sm16, sm17, sm18, sm20", "os3, sm3, sm4, sm8, sm9, sm15, sm18", "os2, sm1, sm3, sm4, sm9, sm10, sm11, sm14, sm15, sm16, sm17, sm18"),
...
All four models were checked for the CPH assumptions. All but the first
model failed the assumptions, even after adapting the covariate using
stratification and/or removal. As such, the first model was the one
selected for the continuing analyses.

While model one does have initially contain all 24 covariates, there are
...
using model one without sm16 and sm19. The below tables provide the
...
The forest plot for model one is shown below. The covariates that are
...
plot_model(cox1.1,
           ...
           colors = "red")+
  theme_bw()+
  labs(title = "Estimated Effects of Covariates on Hazard Ratio")+
  xlab(label = "Covariates")+
  ylab(label = "Estimated Effect on Hazard Ratio")
```
...
The model's final concordance index of .696 indicates that the
calculated confidence interval is fairly accurate, but it is still
subject to a substantial margin for error.
...
While the concordance index of .696 achieved in this analysis
demonstrates the model is fairly accurate, there is still room for
improvement. Survival models resulting in a concordance index of .9 or
higher are highly accurate and would provide a tighter, more accurate
confidence interval for the longevity of engines. After attempting
transformations of the data and creating models with many different
combinations of covariates, the final c-index that was calculated is the
highest value achieved where the covariates were able to meet the
proportional hazards model.

As discussed during earlier, a weakness of the Cox proportional hazards
model is the necessity to meet the proportional hazards assumption.
Given the difficulty of meeting the PH assumption, a different survival
analysis would have better suited this dataset.
